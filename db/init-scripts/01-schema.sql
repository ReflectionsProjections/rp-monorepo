...
CREATE TABLE public."magicLinkTokens" (
...
CREATE UNIQUE INDEX "authInfo_email_normalized_key"
    ON public."authInfo" (lower(btrim("email")));
CREATE INDEX "magicLinkTokens_subjectEmail_idx"
    ON public."magicLinkTokens" (lower(btrim("subjectEmail")));
CREATE INDEX "magicLinkTokens_expiresAt_idx"
...
-- Consume a magic link only when all token conditions match.
...
-- Create one base account for each normalized email address.
...
-- Complete attendee registration in one database transaction.
...
-- Consume a legacy sponsor code only when its stored hash still matches.
