...
async function main() {
    ...
    const { data: existing } = await SupabaseDB.CUSTOM_TOPICS.select('topicName')
    ...
    const { data: staffRoles, error: staffErr } = await SupabaseDB.AUTH_ROLES.select('userId')
        .eq('role', 'STAFF')
        .throwOnError();
    if (staffErr) throw staffErr;

    const staffUserIds = (staffRoles ?? []).map((r: { userId: string }) => r.userId);
    if (staffUserIds.length === 0) {
        console.log('No staff userIds found. Nothing to subscribe.');
        process.exit(0);
    }

    const { data: userDevices } = await SupabaseDB.NOTIFICATIONS.select('userId, deviceId')
