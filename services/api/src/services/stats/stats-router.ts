...
statsRouter.get('/check-in', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
    const { data: checkinEvents } = await SupabaseDB.EVENTS.select('eventId')
    ...
    const checkinEventIds = checkinEvents.map((event: { eventId: string }) => event.eventId);

    const { data: attendanceRecords } = await SupabaseDB.EVENT_ATTENDANCES.select('attendee')
        .in('eventId', checkinEventIds)
        .throwOnError();

    const uniqueAttendees = new Set(
        attendanceRecords?.map((record: { attendee: string }) => record.attendee) || [],
    );
...
statsRouter.get('/merch-item/:PRICE', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
    const schema = z.object({
        PRICE: z.coerce.number().int().gte(0, { message: 'PRICE must be non-negative' }),
    });
...
statsRouter.get('/priority-attendee', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
...
statsRouter.get('/attendance/:N', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
    const schema = z.object({
        N: z.coerce.number().int().gt(0, { message: 'N must be greater than 0' }),
    });
    ...
    const { data: events } = await SupabaseDB.EVENTS.select('attendanceCount')
    ...
    const attendanceCounts =
        events?.map((event: { attendanceCount: number }) => event.attendanceCount) || [];
...
statsRouter.get(
    ...
    async (req, res) => {
        const { data: allRegistrations } = await SupabaseDB.REGISTRATIONS.select(
...
statsRouter.get('/registrations', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
...
statsRouter.get(
    ...
    async (req, res) => {
        ...
        const { data: event } = await SupabaseDB.EVENTS.select('attendanceCount')
            .eq('eventId', eventId)
            .maybeSingle()
            .throwOnError();
        ...
        return res.status(StatusCodes.OK).json({ attendanceCount: event.attendanceCount });
    },
);
...
statsRouter.get('/tier-counts', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
...
statsRouter.get('/tag-counts', RoleChecker([Role.enum.STAFF], false), async (req, res) => {
    const { data } = await SupabaseDB.ATTENDEES.select('tags').throwOnError();
...
statsRouter.get(
    ...
    RoleChecker([Role.enum.STAFF], false),
    async (req, res) => {
        const { data } = await SupabaseDB.REDEMPTIONS.select('item').throwOnError();
        ...
        data?.forEach((redemption: { item: TierType }) => {
            if (redemption.item) {
                itemCounts[redemption.item] = (itemCounts[redemption.item] || 0) + 1;
            }
        });
...
statsRouter.get(
    ...
    async (req, res) => {
        const schema = z.object({
            N: z.coerce.number().int().gte(0, { message: 'N must be greater equal than 0' }),
        });
        ...
        const { data: attendanceRecords } =
            await SupabaseDB.ATTENDEE_ATTENDANCES.select('eventsAttended').throwOnError();

        const countAtLeastN =
            attendanceRecords?.filter(
                (record: { eventsAttended: string[] }) => record.eventsAttended.length >= n,
            ).length ?? 0;
