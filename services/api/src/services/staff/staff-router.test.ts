...
beforeEach(async () => {
    ...
    await SupabaseDB.STAFF.insert(StaffValidator.parse(TESTER_STAFF)).throwOnError();
    await SupabaseDB.STAFF.insert(StaffValidator.parse(OTHER_STAFF)).throwOnError();
...
describe('GET /staff/:EMAIL', () => {
    it.each([
        ...
        async ({ getter }) => {
            const response = await getter(`/staff/${TESTER_STAFF.email}`).expect(StatusCodes.OK);
            expect(response.body).toMatchObject(TESTER_STAFF);
        },
    );

    it('should return an unauthorized error for an unauthenticated user', async () => {
        await get(`/staff/${TESTER_STAFF.email}`).expect(StatusCodes.UNAUTHORIZED);
    });

    it('should return not found for a non-existent user', async () => {
        const response = await getAsAdmin(`/staff/${NON_EXISTENT_EMAIL}`).expect(
...
describe('POST /staff/', () => {
    ...
    it('should return UNAUTHORIZED for an unauthenticated user', async () => {
        await post('/staff/').send(NEW_STAFF_VALID).expect(StatusCodes.UNAUTHORIZED);
    });

    it('should return FORBIDDEN for STAFF user', async () => {
        await postAsStaff('/staff/').send(NEW_STAFF_VALID).expect(StatusCodes.FORBIDDEN);
    });
    ...
    it.each(invalidPayloads)('should return BAD_REQUEST when $description', async ({ payload }) => {
        await postAsAdmin('/staff/').send(payload).expect(StatusCodes.BAD_REQUEST);
    });
    ...
    it('should ignore extra fields in the payload', async () => {
        ...
        await postAsAdmin('/staff/').send(payloadWithExtra).expect(StatusCodes.CREATED);
...
describe('DELETE /staff/:EMAIL', () => {
    ...
    it('should delete existing staff member and return NO_CONTENT', async () => {
        await delAsAdmin(`/staff/${TESTER_STAFF.email}`).expect(StatusCodes.NO_CONTENT);
    ...
    // auth tests
    it('should return UNAUTHORIZED for an unauthenticated user', async () => {
        await del(`/staff/${TESTER_STAFF.email}`).expect(StatusCodes.UNAUTHORIZED);
    });

    it('should return FORBIDDEN for STAFF user', async () => {
        await delAsStaff(`/staff/${TESTER_STAFF.email}`).expect(StatusCodes.FORBIDDEN);
    });

    // not found test
    it("should return NOT_FOUND when trying to delete a staff member that doesn't exist", async () => {
        const response = await delAsAdmin(`/staff/${NON_EXISTENT_EMAIL}`).expect(
            StatusCodes.NOT_FOUND,
        );
        expect(response.body).toEqual({ error: 'UserNotFound' });
    });
...
describe('POST /staff/check-in', () => {
    ...
    it('fails if meeting is outside check-in window', async () => {
        await SupabaseDB.MEETINGS.update({
            startTime: new Date(
                Date.now() - Config.STAFF_MEETING_CHECK_IN_WINDOW_SECONDS * 1000 * 2,
            ).toISOString(),
        })
    ...
    it('successfully checks in', async () => {
        ...
        await SupabaseDB.MEETINGS.update({
            startTime: new Date(
                Date.now() - (Config.STAFF_MEETING_CHECK_IN_WINDOW_SECONDS / 2) * 1000,
            ).toISOString(),
        })
            .eq('meetingId', MEETING.meetingId)
            .select();
...
describe('POST /staff/:EMAIL/attendance', () => {
    ...
    it('adds attendance successfully', async () => {
        const res = await postAsAdmin(`/staff/${OTHER_STAFF.email}/attendance`).send({
            meetingId: MEETING.meetingId,
            attendanceType: StaffAttendanceTypeEnum.EXCUSED,
        });
