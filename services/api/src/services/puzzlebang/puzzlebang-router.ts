...
puzzlebangRouter.post('/', async (req, res) => {
    const { email, puzzleId } = PuzzlebangCompleteRequestValidator.parse(req.body);

    // Email is in registration, so we need to get the userId from the email and pull from attendee
    const { data: registrationData } = await SupabaseDB.REGISTRATIONS.select('userId')
        .eq('email', email)
        .maybeSingle()
    ...
    const { data: attendeeData } = await SupabaseDB.ATTENDEES.select('puzzlesCompleted')
        .eq('userId', userId)
        .maybeSingle()
        .throwOnError(); // allow 0 rows, which will be gracefully handled
