import { post, postAsUser, postWithAuthorization } from '../../../testing/testingTools';
...
describe('POST /puzzlebang', () => {
    it('should complete puzzle and increment points on brand new attendee', async () => {
        const res = await postWithAuthorization('/puzzlebang', Config.PUZZLEBANG_API_KEY)
        ...
        const { data: updated } = await SupabaseDB.ATTENDEES.select('points, puzzlesCompleted')
    ...
    it.each(['email', 'puzzleId'])("fails if body is missing '%s'", async (missing) => {
