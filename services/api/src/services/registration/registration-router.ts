...
registrationRouter.post('/submit', RoleChecker([]), async (req, res) => {
  ...
  const substitution = {
    ...
    majors:
      registration.majors.length > 0 ? registration.majors.join(', ') : 'N/A',
    minors:
      registration.minors.length > 0 ? registration.minors.join(', ') : 'N/A',
  ...
  if (!existing) {
    ...
    await sendHTMLEmail(
      ...
      Mustache.render(templates.REGISTRATION_UPDATE_CONFIRMATION, substitution)
    )
  }
