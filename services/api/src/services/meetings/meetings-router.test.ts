...
afterAll(async () => {
    await SupabaseDB.MEETINGS.delete().in('meetingId', [TEST_MEETING_1_ID, TEST_MEETING_2_ID]);
});
...
describe('GET /meetings/:meetingId', () => {
    it.each([
        { role: 'ADMIN', description: 'an ADMIN user', getter: getAsAdmin },
        { role: 'STAFF', description: 'a STAFF user', getter: getAsStaff },
    ])('should return correct specific meeting data for $description', async ({ getter }) => {
        const response = await getter(`/meetings/${TEST_MEETING_1.meetingId}`).expect(
            StatusCodes.OK,
        );
        expect(response.body).toMatchObject(EXPECTED_TEST_MEETING_1_RESPONSE);
    });

    it('should return unauthorized for unauthenticated user', async () => {
        await get(`/meetings/${TEST_MEETING_1.meetingId}`).expect(StatusCodes.UNAUTHORIZED);
    });

    it("should return not found if meeting doesn't exist", async () => {
        const response = await getAsAdmin(`/meetings/${UNREAL_MEETING_ID}`).expect(
...
describe('POST /meetings/', () => {
    ...
    it('should create a new meeting for an admin user', async () => {
        ...
        expect(new Date(response.body.startTime).toISOString()).toBe(newMeetingData.startTime);
    ...
    it.each([
        ...
        async ({ requester, expectedStatus }) => {
            const response = await requester().send(newMeetingData).expect(expectedStatus);
...
describe('PUT /meetings/:meetingId', () => {
    it('should allow an admin to edit a meeting', async () => {
        ...
        const response = await putAsAdmin(`/meetings/${TEST_MEETING_1.meetingId}`)
            .send(updatedData)
            .expect(StatusCodes.OK);
        expect(response.body.committeeType).toBe(updatedData.committeeType);
        expect(new Date(response.body.startTime).toISOString()).toBe(updatedData.startTime);
    ...
    it.each([
        ...
        {
            description: 'a STAFF user',
            requester: () => putAsStaff(`/meetings/${TEST_MEETING_1.meetingId}`),
            expectedStatus: StatusCodes.FORBIDDEN,
        },
    ])('should not allow $description to edit a meeting', async ({ requester, expectedStatus }) => {
    ...
    it('should allow admins to edit just one field', async () => {
        ...
        const response = await putAsAdmin(`/meetings/${TEST_MEETING_2.meetingId}`)
            .send(updateDataOneParam)
            .expect(StatusCodes.OK);
        expect(response.body.committeeType).toBe(updateDataOneParam.committeeType);
        expect(response.body.meetingId).toBe(TEST_MEETING_2.meetingId);
        expect(new Date(response.body.startTime).toISOString()).toBe(TEST_MEETING_2.startTime);
    });
});
...
describe('DELETE /meetings/:meetingId', () => {
    it('should allow an admin to delete a meeting', async () => {
        await delAsAdmin(`/meetings/${TEST_MEETING_1.meetingId}`).expect(StatusCodes.NO_CONTENT);
        await getAsAdmin(`/meetings/${TEST_MEETING_1.meetingId}`).expect(StatusCodes.NOT_FOUND);
    });
    ...
    it.each([
        ...
        {
            description: 'a STAFF user',
            requester: () => delAsStaff(`/meetings/${TEST_MEETING_2.meetingId}`),
            expectedStatus: StatusCodes.FORBIDDEN,
        },
    ])(
    ...
    it('should return 404 Not Found if meeting does not exist', async () => {
        await delAsAdmin(`/meetings/${UNREAL_MEETING_ID}`).expect(StatusCodes.NOT_FOUND);
    });
});
