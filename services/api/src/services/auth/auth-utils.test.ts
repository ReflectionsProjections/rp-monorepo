...
describe('updateDatabaseWithAuthPayload', () => {
  ...
  it('should create a new staff user', async () => {
    await SupabaseDB.AUTH_INFO.delete().eq('userId', AUTH_STAFF_USER.userId)
    await SupabaseDB.AUTH_ROLES.delete().eq('userId', AUTH_STAFF_USER.userId)
  ...
  it('should create a new admin user', async () => {
    await SupabaseDB.AUTH_INFO.delete().eq('userId', AUTH_ADMIN_USER.userId)
    await SupabaseDB.AUTH_ROLES.delete().eq('userId', AUTH_ADMIN_USER.userId)
...
describe('generateJWT', () => {
  it('should generate a valid jwt', async () => {
    ...
    const ms = require('ms')
    const time = Math.floor((ms(Config.JWT_EXPIRATION_TIME) as number) / 1000)
