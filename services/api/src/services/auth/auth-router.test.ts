  ...
  describe.each([
    ...
    ({ platform, clientId, clientSecret, hasCodeVerifier }) => {
      ...
      it('should login as a new user with a valid code', async () => {
        ...
        expect(mockOAuth2Client).toHaveBeenCalledWith(expectedOAuthConfig)
        expect(mockGetToken).toHaveBeenCalledWith(expectedGetTokenParams)
        ...
        const { data: roleRows } = await SupabaseDB.AUTH_ROLES.select().eq(
          'userId',
          expected.userId
        )
        expect(roleRows?.map((row: { role: Role }) => row.role)).toEqual(
          expected.roles
        )
      })
