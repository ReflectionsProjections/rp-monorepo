...
import { magicLinkCodeDigestForTest, magicLinkTokenDigestForTest } from './magic-link-service';

const sendTemplateEmail = jest.spyOn(sesUtils, 'sendTemplateEmail').mockResolvedValue();

function lastMagicLinkToken(): string {
    const templateData = sendTemplateEmail.mock.calls.at(-1)?.[2];
    const html = typeof templateData?.body === 'string' ? templateData.body : undefined;
...
describe('POST /auth/magic-links', () => {
    ...
    it('stores only a digest of the opaque token', async () => {
        ...
        const { data: row } = await SupabaseDB.MAGIC_LINK_TOKENS.select().single().throwOnError();
...
describe('POST /auth/magic-links/verify', () => {
    ...
    it('honors a directly assigned CORPORATE role and its revocation', async () => {
        ...
        const resumeToken = await issueWebLink('sponsor@example.com', 'resume-book');
    ...
    it('grants STAFF from the roster on the next sign-in', async () => {
        ...
        expect(payload.roles).toEqual(expect.arrayContaining([Role.Enum.USER, Role.Enum.STAFF]));
    });
    ...
    it('always stores emails normalized and never duplicates an account', async () => {
        ...
        const { data: tokenRows } =
            await SupabaseDB.MAGIC_LINK_TOKENS.select('subjectEmail').throwOnError();
...
describe('POST /auth/magic-links/verify-code', () => {
    ...
    function verifiedPayload(token: string): JwtPayload {
        return jsonwebtoken.verify(token, Config.JWT_SIGNING_SECRET) as JwtPayload;
    }
    ...
    it('stores only a keyed digest of the code', async () => {
        ...
        const { data: row } = await SupabaseDB.MAGIC_LINK_TOKENS.select().single().throwOnError();
        expect(row.codeDigest).toBe(magicLinkCodeDigestForTest('person@example.com', code));
    ...
    it('supersedes the old code and link when a new one is requested', async () => {
        ...
        await post('/auth/magic-links/verify-code')
            ...
            .expect(first.code === second.code ? StatusCodes.OK : StatusCodes.UNAUTHORIZED);
    ...
    it('signs rostered sponsors into the resume book with CORPORATE', async () => {
        ...
        const { code } = await issueFor('Codesponsor@Acme.com', 'web', 'resume-book');
    ...
    it('grants ADMIN from the whitelist through the code path', async () => {
        ...
        const response = await post('/auth/magic-links/verify-code')
            ...
            .expect(StatusCodes.OK);
        expect(verifiedPayload(response.body.token).roles).toContain(Role.Enum.ADMIN);
    });
