...
describe('POST /auth/sponsor/login', () => {
    const mockSendTemplateEmail = jest
        ...
        .mockImplementation((_emailId, _templateId, _templateData) => Promise.resolve());
    const mockCreateSixDigitCode = jest.spyOn(sponsorUtils, 'createSixDigitCode');
    ...
    it('should send a login code', async () => {
        ...
        expect(compareSync(sixDigitCode, `${data.hashedVerificationCode}`)).toBe(true);
    });
    ...
    it('returns the generic response without sending for invalid emails', async () => {
        ...
        const { data } = await SupabaseDB.AUTH_CODES.select().eq('email', email).throwOnError();
