    ...
    const handleBarCodeScanned = async ({ data }: { data: string }) => {
        ...
        try {
            ...
            if (parsedQr.userId === DEMO_ACCOUNT_ID) {
                setSuccessMessage(
                    `Successfully checked in demo user into ${selectedEvent.name}!`,
                )
                setShowSuccess(true)
            ...
            setSuccessMessage(
                `Successfully checked in user into ${selectedEvent.name}!`,
            )
            setShowSuccess(true)
            setTimeout(resetScan, 2000)
        } catch (err: any) {
    ...
    return (
        <SafeAreaView className="flex-1 bg-black">
            <View className="px-4 py-2">
                <Text className="text-white font-bold text-lg mt-10 mb-2">
                    Selected Event:
                </Text>
                <TouchableOpacity
            ...
            <View className="flex-1 relative">
                {!errorOccurred ? (
                    <CameraView
                        ...
                        facing="back"
                        onBarcodeScanned={
                            scanReady && !scanned && !loading && !scanDisabled
            ...
            <Modal visible={errorOccurred} transparent animationType="fade">
                ...
                >
                    <View className="bg-red-900/90 p-6 rounded-lg mx-6 max-w-sm min-w-[200px]">
                        <Text className="text-red-200 text-xl font-bold text-center mb-4">
                            Oops!
                        </Text>
