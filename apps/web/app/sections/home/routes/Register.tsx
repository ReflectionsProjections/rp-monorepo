...
const uploadResume = async (url: string, fields: Record<string, unknown>, file: File) => {
...
const processFinalRegistration = (values: RegistrationValues) => {
    ...
    if (values.dietaryOther !== '') {
        processedValues.dietaryRestrictions = processedValues.dietaryRestrictions.map((item) =>
            item === 'Other' ? values.dietaryOther : item,
        );
    }

    if (processedValues.educationLevel === 'Other' && values.educationOther !== '') {
    ...
    if (processedValues.gender === GENDER_SELF_DESCRIBE && values.genderOther !== '') {
...
const AutosaveDraft = ({ onStatus }: { onStatus: (status: SaveStatus) => void }) => {
    ...
    <ScreenShell>
        <VStack w="100%" maxW="640px" textAlign="center" spacing={0}>
            ...
            <HStack
                ...
                mb="34px"
            >
                <Link href="https://apps.apple.com/us/app/r-p-2025/id6744465190" isExternal>
                    <Image
...
const ClosedScreen = () => (
    <ScreenShell>
        ...
        >
            ...
            <Text fontSize="15.5px" lineHeight="1.7" color="rgba(252,242,246,0.62)" mb="32px">
                Thanks for the interest — registration for R|P 2026 is no longer accepting new
                applications. Walk-ins may still be available at the door during the conference.
            </Text>
...
const Wizard = ({ formik, step, setStep, saveStatus, onSaveAndExit }: WizardProps) => {
    const done = SECTIONS.map((_, index) => sectionComplete(index, formik.values));
    const percent = Math.round((done.filter(Boolean).length / SECTIONS.length) * 100);
    ...
    return (
        ...
        >
            <Box w="100%" maxW="900px">
                {/* header */}
                <Flex align="flex-end" justify="space-between" gap="20px" mb="22px" wrap="wrap">
                    <Box>
                        <Heading
                            fontFamily={TITLE_FONT}
                            fontWeight="400"
                    ...
                    <Flex align="flex-end" gap="16px">
                        <VStack spacing="8px" align="flex-end">
                            <Text fontSize="12px" color={FAINT} letterSpacing="0.04em" minH="18px">
