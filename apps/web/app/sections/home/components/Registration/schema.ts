...
export type RegistrationValues = Omit<RegistrationDraft, 'userId' | 'resume'> & {
...
const baseRegistrationSchema = yup.object({
    ...
    tags: yup.array().of(yup.string().required()).required().min(1, 'Please select at least one'),
    isInterestedPuzzleBang: yup.boolean().required(),
    isInterestedMechMania: yup.boolean().required(),
    school: yup.string().required('School is required'),
    educationLevel: yup.string().required('Education Level is required'),
    majors: yup.array().of(yup.string().required()).required().min(1, 'Please select at least one'),
