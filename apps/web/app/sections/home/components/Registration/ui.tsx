...
import { BODY_FONT, TEXT_COLOR, TITLE_FONT } from '@app/sections/home/components/Auth/AuthCard';
...
export const PAGE_GRADIENT = 'linear-gradient(180deg,#0F062D 0%,#24114C 58%,#170A36 100%)';
...
export const WizardInput = (props: InputProps) => <Input {...inputStyles} {...props} />;
...
export const FieldLabel = ({ children, isRequired }: FieldLabelProps) => (
    <Text fontSize="12px" letterSpacing="0.09em" textTransform="uppercase" color={MUTED} mb="10px">
        {children}
        {isRequired && (
...
export const FieldError = ({ name }: { name: string }) => {
    ...
    const message =
        typeof error === 'string'
            ? error
            : Array.isArray(error)
              ? (error.find((item) => typeof item === 'string') ?? 'Please check this field')
