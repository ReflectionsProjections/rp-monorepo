...
export const StudyInfo: React.FC<StudyInfoProps> = ({ degree, graduationYear, majors, minors }) => {
...
const ResumePopupModal = ({
    ...
    const handleLoadResume = () => {
        ...
        api.get(path('/s3/download/user/:userId', { userId: resume.id }))
            .then((response) => {
    ...
    const handleDownloadResume = async () => {
        ...
        try {
            ...
            saveAs(
                fileResponse.data,
                resume.name ? `${resume.name}-${resume.graduationYear}.pdf` : 'resume.pdf',
            );
        } catch (error) {
