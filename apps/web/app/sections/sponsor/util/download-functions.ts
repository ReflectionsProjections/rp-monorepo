...
export async function downloadResumes(resumes: Resume[], selectedResumes: string[]) {
    ...
    try {
        ...
        if (urls.length === 1) {
            ...
            try {
                const fileResponse = await axios.get<unknown, { data: Blob }>(urls[0] ?? '', {
                    responseType: 'blob',
                });
        ...
        throw new Error('Failed to initiate resume download(s). Please try again later.');
    }
}
