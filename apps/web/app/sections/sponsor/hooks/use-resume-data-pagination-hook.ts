...
export function useResumeDataPaginationHook({
    ...
    const getResumesFiltered = (filterBy: {
        ...
        const lowerCasedSelectedJobInterests = new Set(
            selectedJobInterests.map((jobInterest) =>
                jobInterest.toLowerCase(),
            ),
        )
        ...
        return resumes
            .filter((resume) => {
                ...
                const matchesDegree =
                    ...
                    (resume.degree &&
                        lowerCasedSelectedDegrees.has(
                            resume.degree.toLowerCase(),
                        ))

                const matchesYear =
                    selectedYears.length === 0 ||
                    (resume.graduationYear &&
                        lowerCasedSelectedYears.has(
                            resume.graduationYear.toLowerCase(),
                        ))
                ...
                const matchesJobInterest =
                    ...
                    (resume.jobInterest &&
                        resume.jobInterest.some((interest) =>
                            lowerCasedSelectedJobInterests.has(
            ...
            .sort((a, b) => {
                if (filterBy.sortCol) {
                    ...
                    if (Array.isArray(aValue) && Array.isArray(bValue)) {
                        ...
                        moment.isMoment(aValue) &&
                        moment.isMoment(bValue)
                    ) {
                        return filterBy.sortDirection === "asc"
                            ? aValue.diff(bValue)
                            : bValue.diff(aValue)
        ...
        const yearCounts = filteredResumes.reduce(
            (acc, resume) => {
                if (resume.graduationYear) {
                    acc[resume.graduationYear] =
                        (acc[resume.graduationYear] || 0) + 1
                }
                return acc
            },
            {} as Record<string, number>,
    ...
    return {
        ...
        exportResumes: {
            ...
            downloadResumesCSV:
                resumeSelectionAndDownloadHook.downloadResumesCSV,
        },
    }
}
