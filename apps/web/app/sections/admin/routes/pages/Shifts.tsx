...
const Shifts: React.FC = () => {
    ...
    const { data: staff, isLoading: staffLoading } = usePolling('/staff', authorized);

    const { data: events, isLoading: eventsLoading } = usePolling('/events', authorized);
    ...
    const dates = useMemo(() => {
        ...
        if (shifts) {
            shifts.forEach((shift: Shift) => {
                allDates.add(moment.tz(shift.startTime, 'America/Chicago').format('YYYY-MM-DD'));
            });
        }

        if (events) {
            events.forEach((event: Event) => {
                allDates.add(moment.tz(event.startTime, 'America/Chicago').format('YYYY-MM-DD'));
            });
        }
    ...
    const processedShifts = useMemo(() => {
        ...
        dates.forEach((date) => {
            const dateShifts = shifts.filter(
                (shift: Shift) =>
                    moment.tz(shift.startTime, 'America/Chicago').format('YYYY-MM-DD') === date.key,
            );
            ...
            const calendarShifts: CalendarShift[] = dateShifts.map((shift: Shift) => {
                ...
                let endSlot = (endMoment.hour() - 8) * 2 + Math.floor(endMoment.minute() / 30);
                ...
                const shiftAssignments = assignments
                    .filter((assignment: ShiftAssignment) => assignment.shiftId === shift.shiftId)
    ...
    const processedEvents = useMemo(() => {
        ...
        dates.forEach((date) => {
            const dateEvents = events.filter(
                (event: Event) =>
                    moment.tz(event.startTime, 'America/Chicago').format('YYYY-MM-DD') === date.key,
            );
