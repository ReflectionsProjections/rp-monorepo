...
const Shifts: React.FC = () => {
    ...
    const dates = useMemo(() => {
        ...
        if (shifts) {
            shifts.forEach((shift: Shift) => {
                allDates.add(
                    moment
                        .tz(shift.startTime, "America/Chicago")
                        .format("YYYY-MM-DD"),
                )
            })
        }
    ...
    const processedShifts = useMemo(() => {
        ...
        dates.forEach((date) => {
            ...
            const calendarShifts: CalendarShift[] = dateShifts.map(
                (shift: Shift) => {
                    ...
                    let startSlot =
                        (startMoment.hour() - 8) * 2 +
                        Math.floor(startMoment.minute() / 30)
                    let endSlot =
                        (endMoment.hour() - 8) * 2 +
                        Math.floor(endMoment.minute() / 30)
    ...
    const processedEvents = useMemo(() => {
        ...
        dates.forEach((date) => {
            ...
            const calendarEvents: CalendarEvent[] = dateEvents.map(
                (event: Event) => {
                    const startMoment = moment.tz(
                        event.startTime,
                        "America/Chicago",
                    ...
                    <Tbody>
                        {timeSlots.map((timeSlot, slotIndex) => (
                            <Tr key={timeSlot}>
                                ...
                                {dates.map((date) => {
                                    const dateShifts =
                                        processedShifts[date.key] || []
                                    const dateEvents =
                                        processedEvents[date.key] || []
                                    const shiftsInThisSlot = dateShifts.filter(
                                        (shift) =>
                                            shift.startSlot === slotIndex,
                                    )
                                    const eventsInThisSlot = dateEvents.filter(
                                        (event) =>
                                            event.startSlot === slotIndex,
                                    )
