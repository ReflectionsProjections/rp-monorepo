...
function generateHtmlEmail(mdContent: string, templateName: string) {
    const emailTemplate = rpEmailTemplates.find((template) => template.key === templateName);
    const generateTemplate = emailTemplate ? emailTemplate.templateFn : rpNoneTemplate;
...
function EmailMaker() {
    ...
    const [showExtendedNotifPreview, setShowExtendedEmailPreview] = useState<boolean>(false);

    useEffect(() => {
        api.get('/subscription/lists')
    ...
    const sendMassmail = (values: EmailFormValues, helpers: FormikHelpers<EmailFormValues>) => {
        if (
            !window.confirm(
        ...
        api.post('/subscription/send-email', emailData)
            .then(async (response) => {
