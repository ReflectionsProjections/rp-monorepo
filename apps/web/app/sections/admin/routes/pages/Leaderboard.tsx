...
const Leaderboard: React.FC = () => {
    ...
    const [previewNumberAwards, setPreviewNumberAwards] = useState(`${defaultNumberAwards}`);
    ...
    const fetchDailyLeaderboard = useCallback(
        async (day: string, n?: number) => {
            ...
            try {
                ...
                const transformedData: LeaderboardUser[] = data.leaderboard.map((entry) => ({
    ...
    const handleSetEffectiveNumberAwards = useCallback(
        (inputPreviewValue: number) => {
            if (!inputPreviewValue || !leaderboardUsers || leaderboardUsers.length === 0) {
            ...
            const usersGeqBreakpoint = leaderboardUsers.filter((user) => user.points >= breakpoint);
    ...
    const handleChangePreviewNumber = (event: React.ChangeEvent<HTMLInputElement>) => {
