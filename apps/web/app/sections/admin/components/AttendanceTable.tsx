...
const AttendanceBox = () => {
    ...
    const { data: meetings, isLoading: meetingsLoading } = usePolling('/meetings', authorized);
    ...
    const teamMeetings: Record<CommitteeType, ParsedMeeting[]> = useMemo(() => {
        ...
        return {
            ...
            DESIGN: parsedMeetings
                .filter(
                    (meeting) =>
                        meeting.committeeType === 'DESIGN' || meeting.committeeType === 'FULL TEAM',
                )
                .sort(meetingSortFunction),
            DEV: parsedMeetings
                .filter(
                    (meeting) =>
                        meeting.committeeType === 'DEV' || meeting.committeeType === 'FULL TEAM',
                )
                .sort(meetingSortFunction),
            ...
            {isSmall ? (
                <Flex justify="center" direction="column" align="center">
                    <Menu>
                        ...
                        <MenuList sx={mirrorStyle} maxH="40vh" overflowY="scroll">
                            {(Object.keys(staffTeams) as CommitteeType[]).map((team, index) => (
                                <>
                                    {index !== 0 && <MenuDivider />}
                                    <MenuItem
                                        key={team}
                ...
                <Flex justify="center">
                    <Tabs size="lg" minW="60vw">
                        ...
                        <TabPanels>
                            {(Object.entries(staffTeams) as [CommitteeType, ParsedStaff[]][]).map(
                                ([teamName, team], index) => (
                                    <TabPanel key={index}>
                                        {!loading &&
    ...
    useEffect(() => {
        if (meetings.length > 0) {
            setSelectedMeeting((prev) => {
                const stillExists = prev && meetings.find((m) => m.meetingId === prev.meetingId);
                    ...
                    {!selectedMeeting
                        ? Array.from({ length: 10 }).map((_, i) => (
                              <Tr key={i}>
                                  ...
                                  <Td>
                                      <Skeleton height="20px" speed={authorized ? undefined : 0} />
                                  </Td>
                                  {!isSmall && (
                                      <Td>
                                          <Skeleton
