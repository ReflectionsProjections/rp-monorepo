...
export const ExtendedLeaderboardStats: React.FC<{
    ...
    const qualifiedUsers = leaderboardUsers.filter((user) => user.points >= minimumPointsThreshold);

    // Calculate how many will be promoted to each tier after prizes
    const willPromoteToTier2 = qualifiedUsers.filter((user) => user.currentTier === 'TIER1').length;
    const willPromoteToTier3 = qualifiedUsers.filter((user) => user.currentTier === 'TIER2').length;
    const willPromoteToTier4 = qualifiedUsers.filter((user) => user.currentTier === 'TIER3').length;
    ...
    const attendeesWithPoints = leaderboardUsers.filter((user) => user.points > 0).length;
    const percentageOfTotal =
        totalAttendees > 0 ? Math.round((effectiveNumberAwards / totalAttendees) * 100) : 0;
    ...
    return (
        ...
        >
            ...
            <Text fontSize="sm" color={subtextColor} mt={2}>
                {parseFloat(
                    (minimumPointsThreshold / (leaderboardUsers[0]?.points ?? 1)).toFixed(2),
                ) * 100}
            ...
            <Stack spacing={4} textAlign="left" w="100%" mt={4}>
                ...
                >
                    ...
                    <br />({percentageOfTotal}% of total attendees, {percentageWithPoints}% of
                    attendees with points)
                </Text>
            </Stack>
        </Stack>
    );
