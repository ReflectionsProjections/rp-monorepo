...
const MyShifts: React.FC<MyShiftsProps> = ({ authorized }) => {
    ...
    const sortedShifts = [...myShifts]
        ...
        .sort((a, b) => moment(a.shifts!.startTime).diff(moment(b.shifts!.startTime)));
            ...
            {Object.entries(shiftsByDate).map(([date, shifts]) => (
                <Box key={date}>
                    ...
                    <VStack spacing={2} align="stretch">
                        {shifts.map((shift) => (
                            <Card key={shift.shiftId} size="sm" variant="outline" sx={mirrorStyles}>
                                <CardBody p={3}>
                                    <VStack align="start" spacing={2}>
                                        ...
                                        <HStack justify="space-between" w="full" mt={2}>
                                            <HStack spacing={2}>
                                                <Badge
                                                    colorScheme={
                                                        shift.acknowledged ? 'green' : 'orange'
                                                    }
                                                    size="md"
                                                    fontSize="sm"
                                                    py={1}
                                            ...
                                            <IconButton
                                                ...
                                                size="md"
                                                colorScheme={
                                                    shift.acknowledged ? 'orange' : 'green'
                                                }
                                                variant="ghost"
                                                onClick={() =>
