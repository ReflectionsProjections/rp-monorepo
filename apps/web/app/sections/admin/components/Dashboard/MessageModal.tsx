...
export default function MessageModal({
    ...
    const { isOpen, onOpen, onClose } = disclosureOverride ? disclosureOverride : disclosure;
    const mirrorStyle = useMirrorStyles();
    const toast = useToast();

    function onSubmit({ isEmbed, message, embedFullscreen, embedIframe }: MessageForm) {
        const data: DashboardMessageRequest = !isEmbed
            ? { message }
            : { url: message, fullscreen: embedFullscreen, iframe: embedIframe };
        onClose();
        api.post(
            target !== null ? path('/dashboard/message/:id', { id: target }) : '/dashboard/message',
            data,
        )
            .then(() => {
                toast({
        ...
        <>
            ...
            <Modal isOpen={isOpen} onClose={onClose}>
                ...
                <ModalContent sx={mirrorStyle}>
                    <ModalHeader>
                        Send a message to {target !== null ? `display #${target}` : 'all displays'}
                    </ModalHeader>
                    <ModalBody>
                        <Formik<MessageForm>
