...
import { Card, Flex, Text, Button, Stack, Spacer, useToast, useDisclosure } from '@chakra-ui/react';
...
export default function DisplayCard({ display }: { display: Display }) {
    ...
    function identify() {
        api.post(path('/dashboard/identify/:id', { id }), undefined)
            .then(() =>
                toast({
    ...
    function reload() {
        api.post(path('/dashboard/reload/:id', { id }), undefined)
            .then(() =>
                toast({
    ...
    return (
        <Card key={id} sx={mirrorStyle} w="100%">
            <Flex alignItems="center" w="100%" gap={'3rem'} textAlign={'left'}>
                ...
                <Stack direction="row" spacing={2} flexWrap={{ base: 'wrap', xl: 'nowrap' }}>
