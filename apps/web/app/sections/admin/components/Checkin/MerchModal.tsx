...
const MerchModal: React.FC<MerchModalProps> = ({ isOpen, onClose, selectedAttendee }) => {
    const { data, update } = usePolling(
        path('/attendee/redeemable/:userId', {
            userId: selectedAttendee?.userId ?? '',
        }),
    );
    const [merchToRedeem, setMerchToRedeem] = useState<Partial<Record<Tier, boolean>>>({});
            ...
            <ModalContent>
                ...
                <ModalBody pb={6}>
                    ...
                    {data?.redeemableTiers?.length ? (
                        ...
                        <Text>
                            {selectedAttendee?.name} has no merch items available for redemption.
                        </Text>
                    )}
