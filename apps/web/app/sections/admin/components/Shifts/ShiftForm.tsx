...
const validationSchema = Yup.object({
    ...
    endTime: Yup.string()
        ...
        .test('is-after-start', 'End time must be after start time', function (endTime: string) {
...
const ShiftForm: React.FC<ShiftFormProps> = ({ shift, events = [], onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            const startTime = moment.tz(values.startTime, 'America/Chicago').toISOString();
            ...
            if (shift) {
                ...
                await api.patch(`/shifts/${shift.shiftId}` as '/shifts/:shiftId', shiftData);
                toast({
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (eventId) {
                                                ...
                                                if (selectedEvent) {
                                                    ...
                                                    const formValues =
                                                        form.values as ShiftFormValues;
                                                    if (!formValues.name) {
                                                        const roleLabel =
                                                            shiftRoleOptions.find(
                                                                (opt) =>
                                                                    opt.value === formValues.role,
                                                            )?.label || 'Shift';
