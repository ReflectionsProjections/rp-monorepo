...
function EventCard({ event }: { event: Event | null }) {
    ...
    const startTimeCST = convertToCST(
        event?.startTime ?? "2025-03-01T22:00:00Z",
    )
        ...
        <Card maxW="sm" sx={mirrorStyles}>
            <CardBody>
                ...
                <Stack mt="6" spacing="3">
                    ...
                    <Text>
                        {startTimeCST.format(readable)} -{" "}
                        {endTimeCST.format(readable)}
                    </Text>
                    <Text>
                        (
                        {moment
                            .duration(endTimeCST.diff(startTimeCST))
                            .humanize()}
                        )
                    </Text>
