...
export const useAttendanceViewHook = (attendanceData: StaffAttendance[]) => {
    ...
    const handleLoadData = useCallback(() => {
        ...
        const committeeTypes = Array.from(
            new Set(attendanceData.map((item) => item.committeeType)),
        )
        ...
        while (currentDate.isSameOrBefore(endRangeDate)) {
            ...
            for (const committeeType of committeeTypes) {
                const meetingsInWeek = attendanceData.filter(
                    (item) =>
        ...
        for (let i = 1; i < newWeekData[committeeTypeNames[0]].length; i++) {
            ...
            if (
                item.weekInfo.month != previousItem.weekInfo.month &&
                !previousItem.isHeaderItem
            ) {
                newWeekData[committeeTypeNames[0]][i].isHeaderItem = true
            }
        }

        const weeksWithHeaderItems = newWeekData[committeeTypeNames[0]].filter(
            (item) => item.isHeaderItem,
        )
        if (weeksWithHeaderItems.length > 0) {
            const firstWeekWithHeaderItem = weeksWithHeaderItems[0]
            const firstWeekWithHeaderItemWeekNumber =
                firstWeekWithHeaderItem.weekInfo.weekNumber
            const firstHeaderItemWeekNumber =
                newWeekData[committeeTypeNames[0]][0].weekInfo.weekNumber
            // 2 or more weeks apart
            if (
                firstWeekWithHeaderItemWeekNumber - firstHeaderItemWeekNumber >=
                2
            ) {
