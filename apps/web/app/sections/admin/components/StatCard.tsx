...
type Transformer<T extends GettablePaths> = (data: APIRoutes[T]['GET']['response']) => number;
...
const StatCard = <T extends GettablePaths>({
    ...
    const { data, error, isLoading } = usePolling(endpoint, enabled ?? true, interval);
        ...
        <Stat sx={mirrorStyles}>
            ...
            <StatNumber>
                {isLoading || error ? (
                    ...
                    <AnimatedCounter value={(data !== null ? transformer(data) : 0).toString()} />
                )}
            </StatNumber>
        </Stat>
    );
