...
const linkMap: Record<string, { path: string; role: Role }> = {
    ...
    Stats: { path: '/admin/stats', role: 'ADMIN' },
    Events: { path: '/admin/events', role: 'ADMIN' },
    Meetings: { path: '/admin/meetings', role: 'ADMIN' },
    Roles: { path: '/admin/roles', role: 'SUPER_ADMIN' },
    Sponsors: { path: '/admin/sponsors', role: 'ADMIN' },
    Speakers: { path: '/admin/speakers', role: 'ADMIN' },
    Shifts: { path: '/admin/shifts', role: 'ADMIN' },
    'Check-in': { path: '/admin/checkin', role: 'ADMIN' },
    Massmailer: { path: '/admin/massmailer', role: 'SUPER_ADMIN' },
    Leaderboard: { path: '/admin/leaderboard-view', role: 'SUPER_ADMIN' },
    Attendance: { path: '/admin/attendance-view', role: 'ADMIN' },
    Dashboard: { path: '/admin/dashboard', role: 'ADMIN' },
};
    ...
    return (
        <HStack gap={4}>
            ...
            <IconButton
                ...
                title={isForcedMode ? 'Color mode is fixed by current theme' : 'Toggle dark mode'}
            />

            {/* Color Profile Menu */}
            <Menu isOpen={isColorMenuOpen} onClose={onColorMenuClose}>
                <MenuButton as={Button} position="absolute" opacity={0} pointerEvents="none" />
                <Portal>
                    <MenuList>
...
const Navbar: React.FC<NavbarProps> = ({ roles, loading, displayName }) => {
     ...
     */
    const NavbarLink = ({ children, href }: { children: ReactNode; href: string }) => {
