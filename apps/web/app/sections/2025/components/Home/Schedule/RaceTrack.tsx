...
export function RaceTrack({
    ...
    const [finishLineLocation, setFinishLineLocation] = useState<TrackLocation | null>(null);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < numLocations; i++) {
            ...
            if (i === numLocations - 1) {
                ...
                const { x: finalX, y: finalY } = pathEl.getPointAtLength(t_final * totalLen);
...
function RaceTrackPath({ path, pathRef }: { path: string; pathRef?: React.Ref<SVGPathElement> }) {
    return (
        <path
            ref={pathRef}
            d={path}
