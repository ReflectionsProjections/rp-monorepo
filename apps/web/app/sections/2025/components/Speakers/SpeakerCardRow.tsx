...
import { Box, HStack, IconButton, Text, useMediaQuery, VStack } from '@chakra-ui/react';
...
export default function SpeakerCardRow({ row }: { row: SpeakerRow }) {
    ...
    const { ref: bioTextRef, height: bioTextHeight } = useElementHeight<HTMLParagraphElement>();
    ...
    const displayedSelectedIndex =
        selectedIndex !== null ? Math.min(selectedIndex, row.speakers.length - 1) : null;

    const displayedContent =
        displayedSelectedIndex !== null ? row.speakers[displayedSelectedIndex].bio : null;
    ...
    return (
        ...
        >
            <HStack
                ...
                transition={row.speakers.length === 1 && !speakerSelected ? 'all 0.5s' : 'all 0.3s'}
                mb={{
                    base: 1,
                    md: 3,
                }}
            ...
            >
                {row.speakers.map((speaker, index) => (
                    <SpeakerImageCard
                        ...
                        speaker={speaker}
                        customHeight={speakerSelected ? `${bioTextDisplayedHeight}px` : '150px'}
                ...
                <Box
                    ...
                    transition={
                        row.speakers.length === 1 && !speakerSelected ? 'all 0.5s' : 'all 0.3s'
                    }
                    zIndex={3}
                ...
                {row.speakers.map((speaker, index) => {
                    ...
                    return (
                        <Text
                            ...
                            isTruncated={isSmallScreen ? false : displayedSelectedIndex !== null}
                        ...
                        >
                            {isKeynote && !speakerSelected ? 'Keynote Speaker' : speaker.title}
                        </Text>
                    );
                })}
            </HStack>
