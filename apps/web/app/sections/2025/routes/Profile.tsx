import { Box, HStack, Icon, Image, Link, Spinner, Text, useToast, VStack } from '@chakra-ui/react';
...
const mockAttendee: Attendee = {
    ...
    name: 'John Doe',
    email: 'johndoe@example.com',
    events: ['174a84db-412d-4e24-b0ad-15baa3815151', '8852aff5-015a-40b2-a32b-f1f738db34c8'],
...
export function Profile() {
    ...
    const handleLoadQr = () => {
        setQr('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
    };

    const handleLoadAuthData = () => {
        const role = mockRoleObject;
        setRoleObject(role);
        ...
        if (role.userId) {
            ...
            if (now > rpStartDate && now < rpEndDate) {
                // RP in progress
                const hasPriority =
                    newAttendee[`hasPriority${todayShort}` as keyof typeof newAttendee] || false;
    ...
    return (
        ...
        >
            <Box px={6} pt={6} pb={4} maxW="400px" w="100%" mx="auto">
                ...
                <VStack border={'1px solid #ccc'} p={2} pt={0} borderRadius={'xl'} mt={3} gap={0}>
                ...
                >
                    Prize Tier:{' '}
                    {attendee?.currentTier ? tierToName[attendee.currentTier] : 'Not available'}
                ...
                <Text fontFamily="Magistral" fontSize="lg" fontWeight="bold" mb={2}>
                    Level up your prize tier to unlock more rewards at the end of the event.
                </Text>

                <Text fontFamily="Magistral" fontSize="lg" fontWeight="bold" mb={2}>
                    Attend events and activities to earn points, which advance your prize tier.
                </Text>
                ...
                <Box p={8} bgColor={'#ccc'} borderRadius={'xl'}>
                    {qr ? (
                        <QRCodeSVG value={qr} width="100%" height="100%" bgColor="transparent" />
                    ) : (
                        <Spinner />
                    )}
                </Box>
                ...
                {markers.map(({ label, size, bg, color, fontSize }) => (
                    ...
                    >
                        <Box
                            ...
                            mb={1}
                        >
                            <Text fontSize={fontSize} fontWeight="bold" color={color} ml={'-2px'}>
                                {label}
                            </Text>
                        </Box>
                    </Box>
