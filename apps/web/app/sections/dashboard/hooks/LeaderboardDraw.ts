...
export default function useUpdateAnimationLoop({
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            const { trackDrawSegments, totalDistance } =
                getTrackDrawSegments(TRACK)
            const bitmaps = {
                    ...
                    if (ctx) {
                        ...
                        if (result) {
                            ...
                            for (const [i, pos] of positions.entries()) {
                                const entryRef =
                                    entryRefs.current && entryRefs.current[i]
                                ...
                                entryRef.style.setProperty(
                                    "--drawX",
                                    `${drawTransformed.x}px`,
                                )
                                entryRef.style.setProperty(
                                    "--drawY",
                                    `${drawTransformed.y}px`,
                                )
                                entryRef.style.setProperty(
                                    "--drawAngle",
...
function getCameraFocus(timeSeconds: number) {
    ...
    if (t < PAN_DOWN_PAUSE) {
        ...
        return (
            PAN_DOWN_TO_CAR -
            ((t - PAN_DOWN_PAUSE - PAN_DOWN_TIME - PAN_DOWN_PAUSE) /
                PAN_DOWN_TIME) *
                PAN_DOWN_TO_CAR
        )
    }
...
function drawTrack(
    ...
    for (const segment of trackDrawSegments) {
        ...
        const endDistance = Math.sqrt(
            Math.pow(cameraX - segment.fX, 2) +
                Math.pow(cameraY - segment.fY, 2),
        )
        if (
...
function drawTrees(ctx: CanvasRenderingContext2D, segment: TrackDrawSegment) {
    ...
    if (type == "straight") {
        ...
        for (let i = 0; i < maxTrees; i++) {
            ...
            if (spawn) {
                const dx =
                    treeDistance * Math.cos(treeAngle + perpAngle) * spawn
                const dy =
                    treeDistance * Math.sin(treeAngle + perpAngle) * spawn
                drawTree(
                    ctx,
                    midpointX + nx * spawn + dx,
                    midpointY + ny * spawn + dy,
                )
            }
        }
    } else if (type == "arc") {
        ...
        for (let i = 0; i < maxTrees; i++) {
            ...
            if (spawn) {
                const dx =
                    treeDistance * Math.cos(treeAngle + angleAlongArc) * spawn
                const dy =
                    treeDistance * Math.sin(treeAngle + angleAlongArc) * spawn
                drawTree(
                    ctx,
                    centerX + nx * spawn + dx,
                    centerY + ny * spawn + dy,
...
function drawCars(
    ...
    for (const [i, entry] of leaderboard.entries()) {
        ...
        const driftX =
            ...
            0.1 *
                Math.sin(
                    timeSeconds / carCycles[i].cycleX + carCycles[i].offsetX,
                )
        const driftY =
            0.025 *
