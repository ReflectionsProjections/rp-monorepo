...
export default function useUpdateAnimationLoop({
    ...
    }, [entryRefs, canvasRef, trackPercent, carImages, roadImage, roadSidingImage, leaderboard]);
}

// createImageBitmap isn't supported properly for svgs on some browsers, so we implement it ourselves
async function createBitmapFromImage(image: HTMLImageElement, width: number, height: number) {
...
async function generateCarBitmaps(carImages: Record<IconColor, HTMLImageElement>) {
    ...
    for (const segment of track) {
        const trackDrawSegment: TrackDrawSegment =
            'distance' in segment
                ...
                : getArcTrackDrawSegment(x, y, angle, rng, segment.angle, segment.radius);
...
function getArcTrackDrawSegment(
    ...
    const angleDiff = right ? endDrawAngle - startDrawAngle : startDrawAngle - endDrawAngle;
...
function updateCamera(
    ...
    if (cameraFollowing) {
        const { x, y, angle } = getTrackPosition(trackDrawSegments, totalDistance, focusPosition);
        cameraX = x;
        cameraY = y;
    ...
    const centerX = (cameraFollowing ? FIRST_CAR_CAMERA_X_SCALE : 0.5) * trackPercent;
...
function drawTrack(
    ...
    for (const segment of trackDrawSegments) {
        ...
        if (Math.min(startDistance, endDistance) < RENDER_DISTANCE / Math.min(1, cameraScale)) {
...
function drawFinishLine(ctx: CanvasRenderingContext2D, x: number, y: number, angle: number) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rad(angle));
...
function drawCars(
    ...
    for (const [i, entry] of leaderboard.entries()) {
        ...
        const driftX =
            stagerX + 0.1 * Math.sin(timeSeconds / carCycles[i].cycleX + carCycles[i].offsetX);
        const driftY = 0.025 * Math.sin(timeSeconds / carCycles[i].cycleY + carCycles[i].offsetY);

        // Draw the car
        const carPosition = position + -CAR_SPACING * i;
        const { x, y, angle } = getTrackPosition(trackDrawSegments, totalDistance, carPosition);
        const res = drawCar(ctx, x, y, angle, driftX, driftY, carBitmaps?.[entry.icon]);
