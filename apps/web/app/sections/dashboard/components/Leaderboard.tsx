...
import useUpdateAnimationLoop, { DRAW_CARS_IN_CANVAS } from '../hooks/LeaderboardDraw';

const TOP_CARS_NUMBER = 10;

export default function Leaderboard({ trackPercent }: { trackPercent: number }) {
    const [leaderboard, setLeaderboard] = useState<LeaderboardEntry[] | undefined>(undefined);
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [carImages, setCarImages] = useState<Record<IconColor, HTMLImageElement> | undefined>(
        undefined,
    );
    const [roadImage, setRoadImage] = useState<HTMLImageElement | undefined>(undefined);
    const [roadSidingImage, setRoadSidingImage] = useState<HTMLImageElement | undefined>(undefined);
    ...
    async function loadImages() {
        const loadedEntries = await Promise.all(
            Object.values(IconColors).map(async (color: IconColor) => {
                return new Promise<[IconColor, HTMLImageElement]>((resolve, reject) => {
                    const img = new Image();
                    const coloredSvg = CarSvg.replace(/currentColor/g, ICON_COLOR_TO_COLOR[color]);
        ...
        const images = Object.fromEntries(loadedEntries) as Record<IconColor, HTMLImageElement>;
    ...
    return (
        <Flex flexDirection={'column'} height={'100%'} maxHeight={'100%'}>
            <Flex position={'relative'} minHeight={'0'} flexGrow={'1'} overflow={'hidden'}>
    ...
    return (
        <Box ref={ref}>
            {!DRAW_CARS_IN_CANVAS && (
                ...
                >
                    <Car width={'100%'} height={'100%'} color={ICON_COLOR_TO_COLOR[entry.icon]} />
                </Box>
            )}
        ...
        >
            ...
            >
                ...
                <Box marginRight={'0.5vh'}>
                    <Icon width={'4vh'} height={'4vh'} color={ICON_COLOR_TO_COLOR[icon]} />
                </Box>
